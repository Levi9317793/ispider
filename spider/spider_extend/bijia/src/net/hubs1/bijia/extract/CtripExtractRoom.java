...
public class CtripExtractRoom {

    public static List extraRoom(String url ,String input){
	...
	public static void subExtraRoom(String url ,Element roomElement,Map<String,String> roomMap){
		...
		if(nameRoomElement!=null){
			...
			roomMap.put("source_url", url+"#"+roomCode);
		}
		//床型 class=unexpanded
		...
		if(roomUnElement!=null && roomUnElement.attr("class").contains("unexpanded")){
			...
			if(bedTdNextElement!=null 
				...
				roomMap.put("roomCode",roomMap.get("roomCode")+"_"+ToolCommand.pinyin(bedType, null));
				roomMap.put("source_url", url+"#"+roomMap.get("roomCode"));
			...
			if(roomElement.attr("class").equals("clicked hidden")){
				...
				for(Element liElement : ulElement.children()){
					if(liElement.attr("title").trim().startsWith("建筑面积")){
						...
						if(bedIndex!=-1){
							roomMap.put("bedDesc", bed.substring(bedIndex+1));
						}
					}
					else if(liElement.attr("title").trim().startsWith("可入住人数"))
					...
					if(imgRoomElement!=null){
						for(Element aRoomElement : imgRoomElement.children()){
							...
							if(roomStartIndex>=0 && roomEndIndex>roomStartIndex){
								...
								imgRoomUrl = imgRoomUrl.substring(roomStartIndex, roomEndIndex);
    ...
    public static List<Map<String, String>> splitMap(Map<String,String> roomMap,String url){
        ...
        while(it.hasNext()) {
            ...
            if(key.equals("bedItem1")||key.equals("bedItem2")||key.equals("source_url")
            		|| key.equals("room_code")){
            	
            }
            else if(key.equals("bedType")){
            	String bedType = roomMap.get("bedItem1");
            	copyMap1.put("bedType",bedType);
            	copyMap1.put("roomCode",roomMap.get("roomCode")+"_"+ToolCommand.pinyin(bedType, null));
            	copyMap1.put("source_url", url+"#"+copyMap1.get("roomCode"));
            	bedType = roomMap.get("bedItem2");
            	copyMap2.put("roomCode",roomMap.get("roomCode")+"_"+ToolCommand.pinyin(bedType, null));
            	copyMap2.put("source_url", url+"#"+copyMap1.get("roomCode"));
