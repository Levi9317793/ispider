...
public class ExtractPriceMR implements JobMR{

	
	private static final Log logger = LogFactory.getLog(ExtractPriceMR.class);
		...
		public void map(ImmutableBytesWritable row, Result values,
			...
			for (KeyValue kv : values.list()) {
				if ("base".equals(Bytes.toString(kv.getFamily()))
					...
					try {
						list = CtripPriceExtract.extractPrice(new String(kv.getValue()), url);
					} catch (Exception e) {
						e.printStackTrace();
					}
	...
	public boolean mapStart(){
		...
		try{
			...
			job.setJarByClass(ExtractPriceMR.class); // class that contains mapper
