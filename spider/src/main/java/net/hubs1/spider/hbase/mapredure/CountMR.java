...
import net.hubs1.spider.hbase.mapredure.SendMessageMR.SendMapper;
...
//	 protected static Set<String> queueNameSet=new HashSet<String>();
	 ...
	 public static class Draw extends TableMapper<ImmutableBytesWritable, Put> {
...
//			 	String rowkey=new String(row.get());
			 	...
			 	String country=new String(value.getValue("prop_base".getBytes(), "country".getBytes()));
			 	String city=new String(value.getValue("prop_base".getBytes(), "city".getBytes()));
			 	
//			 	KeyOut ko=new KeyOut();
//			 	context.write(ss, new Put());
		 }
	 ...
	 public static class Count extends TableReducer<Text, IntWritable, ImmutableBytesWritable> {
	...
	public static void main(String[] args) throws Exception {
		...
		try{
			...
			job.setMapSpeculativeExecution(false); //单datenode执行，true为多节点执行。这里指备份执行
